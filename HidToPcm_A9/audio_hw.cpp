                       ...
                       size_t bytes)
{
	...
	ret = access("/dev/hidraw0",F_OK);
	if(ret!=0)
        return 1;

	if (in->standby) { //if standby tell hidraw is standby,we can read data
		 //ALOGD("Now open hidraw0");
		hid_fd = open("/dev/hidraw0", O_RDWR); 
        if (hid_fd < 0) {
            //ALOGE("HIDRAW OPEN FAILED"); 
        }

        in->standby = false;
	}

	//while ((len = read(hid_fd, buf, 101 )) >0) {
		/*
		for(i=0; i < len; i++) {
			ALOGD("%02x ",buf[i]);
		}
		ALOGD("\n");
		*/
	if((len = read(hid_fd, buf, 101 )) >0) {
		if (buf[0] == 0x08) {
		//if (buf[0] == 0x08 && buf[1] != 0x00 && buf[2] != 0x00 && buf[3] != 0x00) {
			decode(&mg, buf+1, 200, buffer);
			bytes = 400;
			//return 1024;//when finish decode return the buffer?
		} else if ((buf[0] == 0x01 && buf[1] == 0xCC && buf[2] == 0xCC && buf[3] == 0x00)) {
		//} else if ((buf[0] == 0x01 && buf[1] == 0x00 && buf[2] == 0x00 && buf[3] == 0x00)) {
			//ALOGE("need decode data\n");
			mg.valprev = 0;
			mg.index = 0;
			memset(buffer, 0, 400);
			bytes = 0;
